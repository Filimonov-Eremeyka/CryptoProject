...
from datetime import datetime, timedelta, timezone   # +timezone для DeprecationWarning
...
class LoaderGUI(tk.Tk):
    def __init__(self):
        ...
        symbol_cb = ttk.Combobox(self, textvariable=self.symbol_var, values=self.tickers)
        symbol_cb.current(0)   # выбрать первый
        symbol_cb.pack()
    ...
    def download(self):
        ...
        tf = self.tf_var.get()
        start = datetime.strptime(self.start_cal.get(), "%Y-%m-%d").replace(tzinfo=timezone.utc)
        end = datetime.strptime(self.end_cal.get(), "%Y-%m-%d").replace(tzinfo=timezone.utc)
